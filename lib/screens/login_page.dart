    ...
    try {
      ...
      if (_manualLoginMode) {
        ...
        if (!didAuth) {
          setState(() => _loading = false);
          ScaffoldMessenger.of(ctx).showSnackBar(
            const SnackBar(content: Text('Verifikasi Face ID dibatalkan/gagal')),
          );
          return;
        }
      }
            ...
            child: Row(
              children: [
                CircleAvatar(
                  radius: 26,
                  backgroundImage:
                      (snap.hasData && snap.data != null) ? MemoryImage(snap.data!) : null,
                  backgroundColor: const Color(0xFFEDE1FF),
                  child: (snap.data == null)
                      ? const Icon(Icons.person, color: Color(0xFF6A35CC), size: 28)
                      : null,
                ),
